...
const service = axios.create({
  ...
  * */
  ...
  // baseURL: 'http://120.76.194.85:9091/',
  timeout: 5000
});
...
//   config.headers.Authorization = window.sessionStorage.getItem('quantaToken')
...
service.interceptors.request.use(
  config => {
    ...
    //   // 登录授权 请求验证是否有token  需要授权的 API ，必须在请求头中使用 `Authorization` 字段提供 `token` 令牌
    // console.log("axios.interceptors.request")
    config.headers.token = window.sessionStorage.getItem('quantaToken')
    return config;
  },
  error => {
    // 请求错误时做些事
...
service.interceptors.response.use(
  response => {
    ...
    // 单点登录功能，如果在别处登陆了就会返回400

    // console.log(response.config.url == "http://120.76.194.85:9091/admin/login")
